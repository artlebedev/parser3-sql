...
*/
static const char *RCSId="$Id: parser3odbc.C,v 1.26.2.1 2008/06/30 10:13:04 misha Exp $"; 
			...
			if(strncasecmp(statement, "select", 6)==0
				...
				TRY {
					rs.Open(
						...
						statement,
						CRecordset::executeDirect|CRecordset::readOnly
					);
				} CATCH_ALL (e) {
...
#define CHECK(afailed) if(afailed) services._throw(sql_error)
				...
				CDBVariant v;
				CString s;
				while(!rs.IsEOF() && (!limit || row<limit)) {
